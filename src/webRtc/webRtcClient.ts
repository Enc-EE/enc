...
export class WebRtcClient {
    ...
    private connectSigServe = () => {
        ...
        this.http.post(this.sigServeUrl + "connectstart/" + this.serverName + "/" + processName, null)
            .then(response => {
    ...
    private waitForConnectOffer = () => {
        this.http.get(this.sigServeUrl + "connectoffer/" + this.serverName + "/" + this.processName)
            .then(response => {
                ...
                this.rtcPeerConnection.setRemoteDescription(JSON.parse(JSON.parse(response))).then(() => {
                    this.rtcPeerConnection.createAnswer().then(
                        (desc) => {
                            ...
                            this.http.post(this.sigServeUrl + "connectanswer/" + this.serverName + "/" + this.processName, JSON.stringify(JSON.stringify(desc)))
                                .then(response => {
    ...
    private checkCandidates = () => {
        ...
        if (this.serverName && this.processName) {
            this.http.get(this.sigServeUrl + "servercandidate/" + this.serverName + "/" + this.processName)
                .then(response => {
