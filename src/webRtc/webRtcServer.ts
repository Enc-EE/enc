...
export class WebRtcServer {
    ...
    private createSigServe() {
        ...
        this.http.post(this.sigServeUrl + "create/" + serverName, null)
            .then(response => {
    ...
    private waitForConnectStart = () => {
        this.http.get(this.sigServeUrl + "connectstart/" + this.serverName)
            .then(response => {
                this.processName = JSON.parse(response);
                this.rtcPeerConnection.createOffer().then(
                    (desc) => {
                        this.rtcPeerConnection.setLocalDescription(desc);
                        this.http.post(this.sigServeUrl + "connectoffer/" + this.serverName + "/" + this.processName, JSON.stringify(JSON.stringify(desc)))
                            .then(response => {
    ...
    private waitForConnectAnswer = () => {
        this.http.get(this.sigServeUrl + "connectanswer/" + this.serverName + "/" + this.processName)
            .then(response => {
    ...
    private checkCandidates = () => {
        ...
        if (this.serverName && this.processName) {
            this.http.get(this.sigServeUrl + "clientcandidate/" + this.serverName + "/" + this.processName)
                .then((response) => {
